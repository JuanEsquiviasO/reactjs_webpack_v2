...
export default class Login extends Component {
	...
	handleOnSubmit(e) {
		...
		login( this.email.value, this.password.value )
			.catch( err => this.setState( this.setMessage(`Invalid user or password: ${err.message}`) ) )
	}
	...
	resetPassword() {
		resetPassword( this.email.value )
			.then( () => this.setState( this.setMessage(`An email has been sent to <b>${this.email.value}</b> reset your password.`) ) )
			.catch( err => this.setState( this.setMessage(`The email ${this.email.value} is not registered.`) ) )
	}
					...
					<input type="password" placeholder="Password" ref={ password => this.password = password } />
					{
						...
						<div className="u-error">
							<p>
								...
								<a href="#" onClick={this.resetPassword} className="alert-link">Did you forget password?</a>
							</p>
						</div>
					}
